...
const AttendeesAvatars: React.FC<AttendeesAvatarsProps> = ({
  ...
  const displayCount = hasAttendeesData ? attendees.length : (fallbackCount ?? 0)
      ...
      >
          ...
          {hasAttendeesData ? (
            <>
              {visibleAttendees.map((attendee, index) => (
                ...
                >
                  <Avatar alt={attendee.displayName || 'User'} className="h-8 w-8">
                    <AvatarImage
                      source={{
                    ...
                    />
                    <AvatarFallback>
                      <Text className="text-xs">{attendee.displayName.charAt(0) || 'U'}</Text>
                    </AvatarFallback>
                  </Avatar>
                </View>
              ))}
        ...
        <View className="ml-3">
          <Text className="text-sm text-gray-500">
            {current?.$id
              ? `${displayCount} ${displayCount === 1 ? t('events.attendees.friendAttending') : t('events.attendees.friendsAttending')}`
              : null}
          </Text>
        </View>
      </TouchableOpacity>
