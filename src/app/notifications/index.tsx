...
export default function NotificationsPage() {
  ...
  >({
    ...
    queryFn: async ({ pageParam = 0 }) => {
      try {
        ...
        const response: NotificationsDocumentsType[] = JSON.parse(data.responseBody)

        return response
      } catch (error) {
        console.error(error)
        showAlert('FAILED', 'Failed to fetch notifications. Please try again later.')
  ...
  const renderItem = useCallback(
    ({ item }: { item: NotificationsDocumentsType }) => <NotificationItem notification={item} />,
    [],
  )
