...
export default function AnnouncementSinglePage() {
  ...
  const { data: announcement, isRefetching } = useQuery<AnnouncementDocumentsType>({
        ...
        <View className={'flex-1 items-center justify-center'}>
          <View className={'native:pb-24 max-w-md gap-6 p-4'}>
            <View className={'gap-1'}>
              ...
              <Muted className={'text-center text-base'}>{t('main.loading')}</Muted>
            </View>
          </View>
        </View>
      </ScrollView>
  ...
  return (
    <ScrollView refreshControl={<RefreshControl refreshing={isRefetching} onRefresh={onRefresh} />}>
        ...
        <View className={'flex-row justify-between gap-4'}>
          <Card className={'flex-1 p-0'}>
            <CardContent className={'p-6'}>
              ...
              <Text className={'text-center'}>{formatDate(new Date(announcement.validUntil))}</Text>
            </CardContent>
          </Card>
        </View>
