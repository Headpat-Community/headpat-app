...
import type { AnnouncementDataType, AnnouncementDocumentsType } from '~/lib/types/collections'
...
export default function AnnouncementsPage() {
  ...
  >({
    ...
    queryFn: async ({ pageParam = 0 }) => {
      try {
        ...
        showAlert('FAILED', 'Failed to fetch announcements. Please try again later.')
  ...
  const renderItem = useCallback(
    ({ item }: { item: AnnouncementDocumentsType }) => <AnnouncementItem announcement={item} />,
    [],
  )

  const keyExtractor = useCallback((item: AnnouncementDocumentsType) => item.$id, [])
  ...
  return (
    <View style={{ flex: 1 }}>
      <FlashList
        ...
        contentInsetAdjustmentBehavior={'automatic'}
        ListFooterComponent={isFetchingNextPage ? <Text>{t('main.loading')}</Text> : null}
      />
    </View>
  )
}
