...
import type { GalleryDocumentsType, GalleryPrefsDocumentsType } from '~/lib/types/collections'

export default function GalleryPage() {
  const { current } = useUser()
  const t = useTranslations()
  const [images, setImages] = React.useState<GalleryDocumentsType[]>([])
  const [imagePrefs, setImagePrefs] = React.useState<Record<string, GalleryPrefsDocumentsType>>({})
    ...
    async (page = 1, append = false) => {
      try {
        ...
        const query = nsfwPreference
          ...
          : [Query.limit(pageSize), Query.offset(offset), Query.equal('nsfw', false)]
        ...
        if (page === 1) {
          ...
          const allDocuments = allGallery.rows as unknown as GalleryDocumentsType[]
        ...
        if (seenItems.size > 0) {
          ...
          const notEqualQueries = Array.from(seenItems).map((id) => Query.notEqual('$id', id))
        ...
        const newDocuments = newGallery.rows as unknown as GalleryDocumentsType[]
        ...
        setSeenItems((prev) => new Set([...prev, ...pageItems.map((item) => item.$id)]))
