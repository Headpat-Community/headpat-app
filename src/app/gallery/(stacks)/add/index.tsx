...
const gallerySchema = z.object({
  name: z.string().trim().min(1, 'Name is required').max(32, 'Name is too long'),
...
export default function GalleryAdd() {
  ...
  const pickImage = async () => {
    try {
      ...
      if (result.assets[0]?.fileSize && result.assets[0].fileSize > maxFileSize) {
  ...
  async function uploadImageAsync() {
    ...
    try {
      ...
      const name = image.fileName ?? `galleryFile.${image.mimeType?.split('/')[1] ?? 'jpg'}`
      const fileData = {
        ...
        <View className={'mx-8 flex-1'}>
          <View className={'flex-1'}>
            <View className={'mt-8'}>
              ...
              {page === 1 && (
                <Muted>
                  You can select an image from your camera roll to upload to the gallery.
                </Muted>
              )}
              {page === 2 && <Muted>Thanks for sharing, please continue to submit.</Muted>}
            </View>
            {page === 1 && (
              <>
                ...
                <View className={'items-center'}>
                  <Muted onPress={() => void openBrowser('https://headpat.place/legal/eula')}>
            ...
            {page === 2 && (
              <View className={'gap-4 py-8'}>
                ...
                <View>
                  ...
                  <Checkbox className={'p-4'} checked={nsfw} onCheckedChange={setNsfw} />
                </View>
                <View>
          ...
          <View style={{ marginBottom: 40 }} className={'gap-4'}>
            ...
            {page === 2 && (
              <>
                <Button disabled={isUploading} onPress={() => void uploadImageAsync()}>
                  <Text>Submit</Text>
                </Button>
