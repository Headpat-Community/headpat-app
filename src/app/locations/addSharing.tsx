...
import type { CommunityDocumentsType, UserDataDocumentsType } from '~/lib/types/collections'
...
export default function AddSharing() {
  ...
  const [selectedItems, setSelectedItems] = React.useState<{ id: string; isCommunity: boolean }[]>(
    [],
  )
    ...
    queryFn: async () => {
      ...
      try {
        ...
        const userQueries = [
          ...
          ...alreadySharedUserIds.map((id) => Query.notEqual('$id', id as string)),
          Query.limit(10),
        ]

        const communityQueries = [
          Query.search('name', debouncedSearchTerm),
          ...alreadySharedCommunityIds.map((id) => Query.notEqual('$id', id as string)),
          Query.limit(10),
        ]
        ...
        showAlert('FAILED', t('location.add.failedToSearch'))
        throw error
      }
    },
  ...
  const handleNext = () => {
    if (selectedItems.length === 0) {
      showAlert('FAILED', t('location.add.failedToSelect'))
      return
    }
    setPage(page + 1)
  }
  ...
  const handleConfirm = async () => {
    ...
    try {
      const result = await Promise.all(
        selectedItems.map((selectedItem) => {
          const documentData = {
            ...
            timeUntil:
              calculateEndTime(selectedDuration)?.toISOString() ?? new Date().toISOString(),
          }
      ...
      showAlert('FAILED', t('location.add.failedToShare'))
      Sentry.captureException(error)
    }
  }
      ...
      {page === 0 && (
        <View>
          <Input
            ...
            placeholder={t('location.add.search')}
          ...
          <View className="flex-1 items-center justify-center">
            <View className="native:pb-24 max-w-md gap-4 p-4">
              <View className="gap-1">
                <H1 className="text-center text-foreground">{t('location.add.selectDuration')}</H1>
                <Muted className="text-center text-base">
                  {t('location.add.selectDurationDescription')}
                </Muted>
              </View>
              <View className="">
                <RadioGroup value={selectedDuration} onValueChange={handleDurationChange}>
                  <RadioGroupItemWithLabel
                    value="1h"
                    label={t('location.add.1hour')}
                    description={t('location.add.1hourDescription')}
                    onLabelPress={() => handleDurationChange('1h')}
                  />
                  <RadioGroupItemWithLabel
                    value="24h"
                    label={t('location.add.24hours')}
                    description={t('location.add.24hoursDescription')}
                    onLabelPress={() => handleDurationChange('24h')}
                  />
                  <RadioGroupItemWithLabel
                    value="7d"
                    label={t('location.add.7days')}
                    description={t('location.add.7daysDescription')}
                    onLabelPress={() => handleDurationChange('7d')}
                  />
                  <RadioGroupItemWithLabel
                    value="30d"
                    label={t('location.add.30days')}
                    description={t('location.add.30daysDescription')}
                    onLabelPress={() => handleDurationChange('30d')}
                  />
                  <RadioGroupItemWithLabel
                    value="unlimited"
                    label={t('location.add.unlimited')}
                    description={t('location.add.unlimitedDescription')}
                    onLabelPress={() => handleDurationChange('unlimited')}
                  />
                  <RadioGroupItemWithLabel
                    value="custom"
                    label={t('location.add.custom')}
                    description={t('location.add.customDescription')}
                ...
                <View className="mt-4 w-full gap-4">
                  <View className="flex-row items-center justify-between">
                    <Text className="text-lg font-semibold">
                      {t('location.add.customDateTime')}
                    </Text>
                    <Button variant="outline" size="sm" onPress={() => setDateOpen(true)}>
                      <Text>{t('location.add.selectDate')}</Text>
                    </Button>
                  </View>
                  <View className="flex-row items-center justify-between">
                    <Text className="text-lg font-semibold">{t('location.add.selectedTime')}</Text>
                    <Button variant="outline" size="sm" onPress={() => setTimeOpen(true)}>
                      <Text>{t('location.add.selectTime')}</Text>
                    </Button>
                  </View>
                  <View className="rounded-lg bg-muted p-4">
                    <Text className="text-center text-lg">
                      {selectedTime?.toLocaleDateString() ?? t('location.add.noDateSelected')} at{' '}
                      {selectedTime?.toLocaleTimeString([], {
                        hour: '2-digit',
                        minute: '2-digit',
                      }) ?? t('location.add.noTimeSelected')}
                    </Text>
                  </View>
                </View>
              )}
        ...
        <>
          <View className="flex-1">
            <View className="native:pb-24 gap-4 p-4">
              <View className="gap-1">
                <H1 className="text-center text-foreground">{t('location.add.confirmSharing')}</H1>
                <Muted className="text-center text-base">
                  {t('location.add.confirmSharingDescription')}
                </Muted>
              </View>
              ...
              <View className="mt-4">
                <Muted className="text-center">
                  {t('location.add.locationWillBeSharedUntil')}
                  {selectedDuration === 'custom'
                    ...
                    : selectedDuration === 'unlimited'
                      ? 'January 1, 2100'
                      : t('location.add.theSelectedDurationExpires')}
                </Muted>
              </View>
            </View>
          </View>
