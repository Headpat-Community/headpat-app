...
export default function MutualLocationsPage() {
  ...
  const [currentEvent, setCurrentEvent] = useState<EventsDocumentsType | null>(null)

  const { data: events } = useQuery<EventsType>({
    queryKey: ['events'],
    queryFn: async () => {
      try {
  ...
  useFocusEffect(
    ...
    }, [current, onRefresh, userLocation, params.focusLatitude, params.focusLongitude]),
  )
  ...
  return (
    <View style={styles.container}>
      <LocationFrontPermissionModal openModal={modalOpen} setOpenModal={setModalOpen} />
      ...
      <Dialog>
        <DialogContent className={'sm:w-[1500px]'}>
          ...
          <DialogFooter>
            <View>
              <Text>Until: {formatDateLocale(new Date(currentEvent?.dateUntil ?? ''))}</Text>
                  ...
                  >
                    <TouchableOpacity>
                      ...
                      >
                        <AvatarImage
                          source={{
                            uri: user.userData.avatarId
                              ? getUserAvatar(user.userData.avatarId)
                              : undefined,
                          }}
                        />
                        <AvatarFallback className={'rounded-xl'}>
                          <Text>
                            {user.userData.displayName ? user.userData.displayName.charAt(0) : 'U'}
                          </Text>
                        </AvatarFallback>
                      </Avatar>
                    </TouchableOpacity>
        ...
        <View style={styles.filterButton}>
          <TouchableOpacity
            className={'h-14 w-14 items-center justify-center rounded-full bg-white shadow'}
