...
import type { EventsDocumentsType, EventsType } from '~/lib/types/collections'
...
export default function NearbyEventsPage() {
  ...
  const [localUserLocation, setLocalUserLocation] = useState<LocationObjectCoords | null>(null)
  ...
  const requestLocationPermission = async () => {
    try {
      ...
      showAlert('SUCCESS', t('location.nearbyEvents.locationGranted'))
    } catch (error) {
      captureException(error)
      showAlert('FAILED', t('location.nearbyEvents.locationFailed'))
    } finally {
      ...
      <View className="flex-1 items-center justify-center px-4">
        <View className="items-center space-y-4">
          ...
          <Muted className="text-center text-lg">
            {userLocation
              ? t('location.nearbyEvents.noEvents')
              : t('location.nearbyEvents.noLocation')}
          </Muted>
          {!userLocation && (
            <View className="mt-6">
              <Button
              ...
              >
                <Text className="font-semibold text-white">
                  {requestingLocation
                    ? t('location.nearbyEvents.requesting')
                    : t('location.nearbyEvents.enableLocation')}
                </Text>
              </Button>
