...
export default function EventPage() {
  ...
  const {
    ...
    queryFn: async () => {
      try {
        ...
        showAlert('FAILED', t('events.failedtofetch'))
        throw error
      }
    },
  ...
  const attendMutation = useMutation({
    ...
    onSuccess: (data) => {
      if (data.type === 'event_attendee_add_success') {
        queryClient.setQueryData(['event', local.eventId as string], (old: any) => ({
          ...old,
          attendees: (Number(old.attendees) || 0) + 1,
          isAttending: true,
        }))
      } else if (data.type === 'event_ended') {
        showAlert('FAILED', t('time.eventHasEnded'))
      } else {
        showAlert('FAILED', t('events.failedToAttend'))
      }
    },
    onError: () => {
      showAlert('FAILED', t('events.failedToFetch'))
  ...
  const unattendMutation = useMutation({
    ...
    onSuccess: (data) => {
      if (data.type === 'event_attendee_remove_success') {
        ...
        showAlert('FAILED', t('events.failedCancelAttendance'))
      }
    },
    onError: () => {
      showAlert('FAILED', t('events.failedToFetch'))
  ...
  useFocusEffect(
    React.useCallback(() => {
      if (!local.eventId) {
        showAlert('FAILED', t('events.idNotFound'))
      }
  ...
  if (isLoading || !isDataComplete) {
    return (
      <>
        <Stack.Screen
          options={{
            headerTitle: event?.title ?? t('events.whatIsThisEvent'),
          }}
        />
        <ScrollView
          className={'flex-1'}
  ...
  return (
    <>
      <Stack.Screen
        options={{
          headerTitle: event.title || t('events.event'),
        }}
      />
      <ScrollView
        refreshControl={
        ...
        {isDataComplete ? (
          <View className={'mx-4 mt-4 gap-4 pb-20'}>
            ...
            <View className={'flex-row justify-between gap-4'}>
              <Card className={'flex-1 p-0'}>
                <CardContent className={'p-6'}>
                  <Text className={'font-bold'}>{t('events.start')}: </Text>
              ...
              <Card className={'flex-1 p-0'}>
                <CardContent className={'p-6'}>
                  <Text className={'font-bold'}>{t('events.end')}: </Text>
            ...
            <View>
              <Card className={'flex-1 p-0'}>
                <CardContent className={'p-6'}>
                  <Text className={'font-bold'}>{t('events.location')}: </Text>
                  <Text>{event.location ? event.location : t('events.noLocationGiven')}</Text>
                </CardContent>
              </Card>
            </View>
            ...
            {hasLocation && (
              <View>
                <TouchableOpacity
                  className={'flex-row items-center justify-center gap-2 rounded-md bg-muted p-4'}
                  onPress={handleViewOnMap}
                >
                  <MapPinIcon size={20} color={theme} />
                  <Text className={'text-center font-semibold'}>{t('events.viewOnMap')}</Text>
                </TouchableOpacity>
              </View>
            )}
            ...
            <View className={'mt-6'}>
              <TouchableOpacity
                className={'items-center justify-center rounded-md bg-primary p-4'}
                onPress={
                  !current
                    ? () => router.push('/login')
                    : isEventEnded
                ...
                disabled={isEventEnded || attendMutation.isPending || unattendMutation.isPending}
              >
                <Text className={'text-center font-bold text-white'}>
                  {isEventEnded
                    ? t('events.eventHasEnded')
                    : event.isAttending
                      ? t('events.cancelAttendance')
                      : event.attendees === 0
                        ? t('events.beFirstAttend')
                        : t('events.attendEvent')}
                </Text>
              </TouchableOpacity>
            </View>
      ...
      >
        ...
        <Text className={'items-center text-center font-bold'}>{attendeesCount}</Text>
      </View>
    </>
  )
}
