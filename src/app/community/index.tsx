...
export default function CommunitiesPage() {
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isRefetching, isLoading } =
    useInfiniteQuery({
      ...
      queryFn: async ({ pageParam = 0 }) => {
        try {
          ...
          showAlert('FAILED', 'Failed to fetch communities. Please try again later.')
  ...
  const renderItem = useCallback(
    ({ item }: { item: CommunityDocumentsType }) => <CommunityItem community={item} />,
    [],
  )

  const keyExtractor = useCallback((item: CommunityDocumentsType) => item.$id, [])

  if (isLoading) {
    return (
      <ScrollView contentInsetAdjustmentBehavior={'automatic'}>
        {Array.from({ length: 10 }).map((_, index) => (
          <View key={`skeleton-${index.toString()}`} className="px-4 py-2">
      ...
      <FlashList
        ...
        ListFooterComponent={
          isFetchingNextPage && hasNextPage ? (
            <Text className="p-4 text-center">{t('main.loading')}</Text>
          ) : null
        }
