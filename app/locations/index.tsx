...
export default function MutualLocationsPage() {
  ...
  const [currentEvent, setCurrentEvent] = useState<EventsDocumentsType | null>(
    null
  )
    ...
    <View style={styles.container}>
      ...
      <Dialog>
        <DialogContent className={"sm:w-[1500px]"}>
          ...
          <DialogFooter>
            <View>
              <Text>
                Until:{" "}
                {formatDateLocale(new Date(currentEvent?.dateUntil ?? ""))}
              </Text>
              <Text>
        ...
        <View style={styles.filterButton}>
          <TouchableOpacity
            className={
              "h-14 w-14 items-center justify-center rounded-full bg-white shadow"
            }
